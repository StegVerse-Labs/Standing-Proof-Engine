...
class ExpectedCorpusFailedSummaryWriterTests(unittest.TestCase):
    ...
    def test_write_expected_corpus_failed_summary_rejects_non_list_failed_checks(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            inventory = root / "inventory.json"
            summary_json = root / "failed.json"
            ...
            inventory.write_text(
                json.dumps({"failed_fixtures": [{"fixture": "expected_results/example.expected.json", "failed_checks": "decision"}]}),
                encoding="utf-8",
            )

            with self.assertRaisesRegex(TypeError, r"failed_fixtures\[0\]\.failed_checks"):
                main([
                    "write_expected_corpus_failed_summary.py",
                    str(inventory),
                    str(summary_json),
    ...
    def test_write_expected_corpus_failed_summary_rejects_nested_failed_check_entry(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            inventory = root / "inventory.json"
            summary_json = root / "failed.json"
            ...
            inventory.write_text(
                json.dumps({"failed_fixtures": [{"fixture": "expected_results/example.expected.json", "failed_checks": [{"name": "decision"}]}]}),
                encoding="utf-8",
            )

            with self.assertRaisesRegex(TypeError, r"failed_fixtures\[0\]\.failed_checks\[0\]"):
                main([
                    "write_expected_corpus_failed_summary.py",
                    str(inventory),
                    str(summary_json),
    ...
    def test_write_expected_corpus_failed_summary_rejects_nested_fixture_label(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            inventory = root / "inventory.json"
            summary_json = root / "failed.json"
            ...
            inventory.write_text(
                json.dumps({"failed_fixtures": [{"fixture": {"name": "expected_results/example.expected.json"}}]}),
                encoding="utf-8",
            )

            with self.assertRaisesRegex(TypeError, r"failed_fixtures\[0\]\.fixture"):
                main([
                    "write_expected_corpus_failed_summary.py",
                    str(inventory),
                    str(summary_json),
    ...
    def test_write_expected_corpus_failed_summary_rejects_nested_path_label(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            inventory = root / "inventory.json"
            summary_json = root / "failed.json"
            ...
            inventory.write_text(
                json.dumps({"failed_fixtures": [{"path": {"name": "expected_results/example.expected.json"}}]}),
                encoding="utf-8",
            )

            with self.assertRaisesRegex(TypeError, r"failed_fixtures\[0\]\.path"):
                main([
                    "write_expected_corpus_failed_summary.py",
                    str(inventory),
                    str(summary_json),
