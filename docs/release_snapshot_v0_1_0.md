...
Commitment Candidate edge-case route
...
4. manifest-authored Commitment Candidate fail-closed testing
5. SDK-bound Commitment Candidate manifest intake
```
...
Commitment Candidate manifest -> PASS / all cases FAIL_CLOSED
Commitment Candidate SDK intake receipt -> PASS
Commitment Candidate expected fixture -> PASS
Commitment Candidate SDK intake expected fixture -> PASS
...
The Commitment Candidate manifest proves that user-authored Transition Table cases can be assembled as data and routed through the existing manifest verifier without adding a bespoke test path per scenario.
...
```text
SPE proof status: PASS
governance result: FAIL_CLOSED
```

This means the artifact proves that the proposed crossing cannot bind consequence when current standing fails. The Commitment Candidate remains non-authorizing; SPE performs the commit-time standing determination.

## SDK-Bound Commitment Candidate Route

The SDK-bound receipt proves that the Commitment Candidate manifest package can be declared through the same SDK intake route as other SPE route packages.
...
The workflow verifies individual samples, the sample manifest, the Commitment Candidate manifest, both SDK intake receipts, expected-result fixtures, expected corpus, JSON exports, reviewer report generation, and formalism tests.
...
SDK-bound Commitment Candidate route: PRESENT
CI coverage: PRESENT
...
```text
v0.2.0 -> live SDK-emitted Commitment Candidate receipt compatibility
v0.3.0 -> master-records reconstruction pointer expansion
