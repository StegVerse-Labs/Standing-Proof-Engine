...
def verify_hash_import(record, repo_root):
    ...
    checks.append(Check("source_event_hash_binding", PASS if event_hash == record.get("source_event_sha256") else FAIL, "source event hash checked"))
    checks.append(Check("destination_event_hash_binding", PASS if event_hash == record.get("destination_event_hash") else FAIL, "destination event hash checked"))
